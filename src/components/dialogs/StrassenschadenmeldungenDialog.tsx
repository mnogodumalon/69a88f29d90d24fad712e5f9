...
interface StrassenschadenmeldungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Strassenschadenmeldungen['fields']) => Promise<void>;
  defaultValues?: Strassenschadenmeldungen['fields'];
  schadenskategorienList: Schadenskategorien[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function StrassenschadenmeldungenDialog({ open, onClose, onSubmit, defaultValues, schadenskategorienList, enablePhotoScan = false, enablePhotoLocation = true }: StrassenschadenmeldungenDialogProps) {
  const [fields, setFields] = useState<Partial<Strassenschadenmeldungen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'straßenschadenmeldungen');
      await onSubmit(clean as Strassenschadenmeldungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Strassenschadenmeldungen['fields']>;
      });
      ...
      if (gps) {
        setFields(f => ({ ...f, gps_koordinaten: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
  ...
  const DIALOG_INTENT = defaultValues ? 'Straßenschadenmeldungen bearbeiten' : 'Straßenschadenmeldungen hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-3">
              ...
              {geoFromPhoto && fields.gps_koordinaten && (
              ...
              {fields.gps_koordinaten?.lat != null && fields.gps_koordinaten?.long != null && (
                <GeoMapPicker
                  lat={fields.gps_koordinaten.lat}
                  lng={fields.gps_koordinaten.long}
          ...
          <div className="space-y-2">
            <Label htmlFor="schadenskategorie">Schadenskategorie</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label htmlFor="foto">Foto des Schadens</Label>
            {fields.foto ? (
          ...
          <div className="space-y-2">
            <Label htmlFor="detaillierte_beschreibung">Detaillierte Beschreibung</Label>
            <Textarea
              id="detaillierte_beschreibung"
              value={fields.detaillierte_beschreibung ?? ''}
              onChange={e => setFields(f => ({ ...f, detaillierte_beschreibung: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
