...
const SCHADENSKATEGORIEN_FIELDS = [
  ...
  { key: 'prioritaetsstufe', label: 'Prioritätsstufe', type: 'lookup/select', options: [{ key: 'niedrig', label: 'Niedrig' }, { key: 'mittel', label: 'Mittel' }, { key: 'hoch', label: 'Hoch' }, { key: 'sehr_hoch', label: 'Sehr hoch' }] },
];
const STRASSENSCHADENMELDUNGEN_FIELDS = [
  ...
  { key: 'schadenskategorie', label: 'Schadenskategorie', type: 'applookup/select', targetEntity: 'schadenskategorien', targetAppId: 'SCHADENSKATEGORIEN', displayField: 'kategoriename' },
  { key: 'foto', label: 'Foto des Schadens', type: 'file' },
  { key: 'schweregrad', label: 'Schweregrad', type: 'lookup/select', options: [{ key: 'gering', label: 'Gering' }, { key: 'maessig', label: 'Mäßig' }, { key: 'erheblich', label: 'Erheblich' }, { key: 'kritisch', label: 'Kritisch' }] },
  { key: 'detaillierte_beschreibung', label: 'Detaillierte Beschreibung', type: 'string/textarea' },
...
const ENTITY_TABS = [
  ...
  { key: 'straßenschadenmeldungen', label: 'Straßenschadenmeldungen', pascal: 'Strassenschadenmeldungen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('schadenskategorien');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'schadenskategorien': new Set(),
    'straßenschadenmeldungen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'straßenschadenmeldungen': return (data as any).strassenschadenmeldungen as Strassenschadenmeldungen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'straßenschadenmeldungen' && fieldKey === 'schadenskategorie') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'straßenschadenmeldungen': return STRASSENSCHADENMELDUNGEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'straßenschadenmeldungen': return {
        create: (fields: any) => LivingAppsService.createStrassenschadenmeldungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateStrassenschadenmeldungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteStrassenschadenmeldungenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'straßenschadenmeldungen' || dialogState?.entity === 'straßenschadenmeldungen') && (
        <StrassenschadenmeldungenDialog
          open={createEntity === 'straßenschadenmeldungen' || dialogState?.entity === 'straßenschadenmeldungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'straßenschadenmeldungen' ? handleUpdate : (fields: any) => handleCreate('straßenschadenmeldungen', fields)}
          defaultValues={dialogState?.entity === 'straßenschadenmeldungen' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'straßenschadenmeldungen' && (
        <StrassenschadenmeldungenViewDialog
          open={viewState?.entity === 'straßenschadenmeldungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'straßenschadenmeldungen', record: r }); }}
          schadenskategorienList={(data as any).schadenskategorien ?? []}
        />
      )}
