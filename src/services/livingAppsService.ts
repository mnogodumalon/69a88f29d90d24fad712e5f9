...
import type { Schadenskategorien, Strassenschadenmeldungen, CreateSchadenskategorien, CreateStrassenschadenmeldungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async createSchadenskategorienEntry(fields: CreateSchadenskategorien) {
    return callApi('POST', `/apps/${APP_IDS.SCHADENSKATEGORIEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schadenskategorien') });
  }
  static async updateSchadenskategorienEntry(id: string, fields: Partial<CreateSchadenskategorien>) {
    return callApi('PATCH', `/apps/${APP_IDS.SCHADENSKATEGORIEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schadenskategorien') });
  }
  ...
  static async getStrassenschadenmeldungen(): Promise<Strassenschadenmeldungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.STRASSENSCHADENMELDUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Strassenschadenmeldungen[];
    return enrichLookupFields(records, 'straßenschadenmeldungen');
  }
  static async getStrassenschadenmeldungenEntry(id: string): Promise<Strassenschadenmeldungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.STRASSENSCHADENMELDUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Strassenschadenmeldungen;
    return enrichLookupFields([record], 'straßenschadenmeldungen')[0];
  }
  static async createStrassenschadenmeldungenEntry(fields: CreateStrassenschadenmeldungen) {
    return callApi('POST', `/apps/${APP_IDS.STRASSENSCHADENMELDUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'straßenschadenmeldungen') });
  }
  static async updateStrassenschadenmeldungenEntry(id: string, fields: Partial<CreateStrassenschadenmeldungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.STRASSENSCHADENMELDUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'straßenschadenmeldungen') });
  }
  static async deleteStrassenschadenmeldungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.STRASSENSCHADENMELDUNGEN}/records/${id}`);
  }
